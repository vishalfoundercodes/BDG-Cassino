...
// export default function DepositNotRecieve() {
//   return (
//     <div>
...
function DepositNotRecieve() {
  ...
  useEffect(() => {
    const handleClickOutside = (event) => {
      if (
        modalSecondRef.current &&
        !modalSecondRef.current.contains(event.target)
      ) {
  ...
  const depositHistory = async (t) => {
    ...
    try {
      let res;
      let apiUrl = `${apis?.deposit_pending_list}${userId}`;
      ...
      <div className="w-full bg-white">
        ...
        <div className="hide-scrollbar overflow-x-auto py-3 mx-3">
          <div className="flex gap-2 text-xsm font-bold">
            {/* <div
              className={`w-30 py-3 flex-shrink-0 flex items-center justify-between shadow-lg rounded-lg ${
                activeModal === -1
            ...
            {payMethod &&
              payMethod?.map((item, i) => (
                <div
                  key={i}
                  className={`w-32 py-3 flex-shrink-0 flex items-center justify-between shadow-lg rounded-lg ${
                ...
                >
                  <img
                    className="w-${item?.type===2?4:2} h-8"
                    src={item?.image}
                    alt="UPI Payment"
                  />
        ...
        {/* <div className="grid grid-cols-2 gap-3 mt-3 mx-3">
          <button
            ...
            className="bg-redLight text-black rounded-md text-xs font-bold py-4 px-2 flex justify-between items-center shadow-md"
          >
            <p>
              {modalFirstValue === 0
                ? "All"
          ...
          <button className="bg-redLight text-black rounded-md text-xs font-bold py-4 px-2 flex justify-center items-center shadow-md">
            <input
        ...
        <div className="px-3 mt-3">
          {depositHistoryData
            ...
            ?.map((item, i) => (
              <div className="bg-white rounded-lg p-2 mb-4" key={i}>
                <div className="flex text-black justify-between items-center">
                  <p className="bg-green text-white rounded-lg px-3 py-0.5">
                    Deposit
                  </p>
                  <p className="text-xsm text-[#5A3633] font-semibold">
                    {item.status === 1
                      ? "Pending"
                      : item?.status === 2
                      ? "Paid"
                ...
                <div className="flex mt-3 text-black justify-between items-center">
                  <p className="text-xsm font-bold">Balance</p>
                  <p className="text-xsm font-semibold text-[#D9AC4F]">
                    ₹{item?.cash}
                  </p>
                </div>
                <div className="flex mt-4 text-black opacity-55 justify-between items-center">
                ...
                <div className="flex my-4 text-black opacity-55 justify-between items-center">
                  <p className="text-xsm font-bold">Order Number</p>
                  <p className="text-xsm flex items-center text-black font-semibold">
                    {item?.order_id} &nbsp;
                    <PiCopyLight
                ...
                {item.status === 1 && (
                  <button
                    className="w-full bg-[#E9BE00] text-white rounded-full py-1 font-semibold"
                    onClick={() => {
                      navigate("/customerservices/depositNotRecieveform", {
                        state: {
                          depositOrderNumber: item?.order_id,
                          orderAmount: item?.cash,
                        },
                      });
                    }}
                  >
            ...
            >
              ...
              <div className="flex flex-col gap-2">
                <button
                  ...
                  className={`${
                    modalFirstValue === 0 ? "text-black" : "text-customlightbtn"
                  }`}
                >
                  All
                </button>
                <button
                  ...
                  className={`${
                    modalFirstValue === 1 ? "text-black" : "text-customlightbtn"
                  }`}
                >
                  To be Paid
                </button>
                <button
                  ...
                  className={`${
                    modalFirstValue === 2 ? "text-black" : "text-customlightbtn"
                  }`}
                >
                  Paid
                </button>
                <button
                  ...
                  className={`${
                    modalFirstValue === 3 ? "text-black" : "text-customlightbtn"
                  }`}
                >
                  Rejected
                </button>
...
export default DepositNotRecieve;
