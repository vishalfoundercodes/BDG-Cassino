...
export default function DepositNotRecieveForm() {
  ...
  const handleSubmit = async () => {
    ...
    if (!imageFile)
      return toast.error("Please upload deposit proof receipt image");
    ...
    try {
      ...
      const res = await axios.post(apis.deposit_Problem, formData, {
        headers: {
    ...
    <div className="bg-[#F8F9FF] p-6 max-w-md mx-auto space-y-6">
      ...
      <div>
        <label className="block text-gray text-sm font-medium mb-1">
          Deposit Order Number<span className="text-[#DE4039]">*</span>
        </label>
        <input
          type="text"
      ...
      <div>
        <label className="block text-gray text-sm font-medium mb-2">
          Deposit proof receipt detail<span className="text-[#DE4039]">*</span>
        </label>
