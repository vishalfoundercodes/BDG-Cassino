...
const AddBankAccountDetails = () => {
    ...
    const addAccountdetailsHandler = async (data) => {
        ...
        console.log('daraaaa payload :',payload)
        ...
        try {
          ...
          if (err?.status === "400" || err?.status === 400) {
            ...
            // toast.error(
            //   err.response.data.message || "Error adding bank account details"
            // );
    ...
    return (
      <div className="min-h-screen  flex flex-col items-center justify-start pb-10 pt-2 px-3">
        {loading && <Loader setLoading={setLoading} loading={loading} />}
        {/* Alert */}
        <div className="w-full max-w-md bg-redLight text-white rounded-full px-2 py-1 mt-1">
        ...
        >
          ...
          <div className="mb-6">
            ...
            <input
              ...
              className="w-full text-xsm text-white placeholder:font-bold outline-none mt-2 px-4 py-3 border border-redLight rounded-lg bg-redLight"
            />
            {errors.name && (
              <p className="text-bg2 text-xs">{errors.name.message}</p>
            )}
          </div>
          {/* Bank upi id */}
          {/* <div className="mb-6">
            ...
            />
            {errors.upi_id && (
              <p className="text-bg2 text-xs">{errors.upi_id.message}</p>
            )}
          </div> */}
          ...
          <div className="mb-6">
            ...
            />
            {errors.ifsc_code && (
              <p className="text-bg2 text-xs">{errors.ifsc_code.message}</p>
            )}
          </div>
          ...
          <div className="mb-6">
            ...
            />
            {errors.bank_name && (
              <p className="text-bg2 text-xs">{errors.bank_name.message}</p>
            )}
          </div>
          ...
          <div className="mb-6">
            ...
            />
            {errors.branch && (
              <p className="text-bg2 text-xs">{errors.branch.message}</p>
            )}
          </div>
          <button
            type="submit"
    ...
    ${
      isValid ? "bg-gradient-to-r from-[#EDD188] to-[#C79744]" : "bg-[#CBCDDB]"
    }`}
            disabled={!isValid}
          >
            Save
